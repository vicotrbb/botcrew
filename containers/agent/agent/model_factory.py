"""Agno model provider factory for the agent container.

Standalone copy of the orchestrator's model_provider.py logic. The agent
container is an independent deployable unit and must not import from the
botcrew orchestrator package.

Maps (provider, model_name, secrets) to configured Agno model instances.
Supports OpenAI, Anthropic, Ollama natively, and Z.ai/GLM via
...
def create_model(
    ...
    if env_key:
        ...
        if not api_key:
            raise ValueError(
                f"API key '{env_key}' not configured. "
                "Add it via the secrets API."
            )
